...
async def ask_version():
    """异步获取版本信息"""
    print(f"[{time.strftime('%H:%M:%S')}] 开始获取版本信息...")
    async with httpx.AsyncClient() as client:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] 版本信息获取成功:")
        print(result)
        return result

async def get_profile():
    """异步获取用户资料"""
    print(f"[{time.strftime('%H:%M:%S')}] 开始获取用户资料...")
    # 模拟这个API需要更长的时间
    async with httpx.AsyncClient() as client:
        ...
        result = response.json()
        print(f"[{time.strftime('%H:%M:%S')}] 用户资料获取成功:")
        print(result)
        return result
...
async def main():
    ...
    for completed_task in asyncio.as_completed(tasks):
        ...
        # 结果已经在各自的函数中输出，这里不需要额外处理
    
    print(f"[{time.strftime('%H:%M:%S')}] 所有API调用已完成")

if __name__ == "__main__":
    print(f"[{time.strftime('%H:%M:%S')}] 开始并发调用API...")
    asyncio.run(main())
