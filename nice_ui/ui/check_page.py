...
from vendor.qfluentwidgets import (InfoBar, InfoBarPosition, PrimaryPushButton,
...
class CudaCheckPage(QWidget):
    ...
    def check_cuda_install(self):
        if torch.cuda.is_available():
            ...
            self.show_info('CUDA 安装', f'CUDA 已正确安装，版本为 {cuda_version}', InfoBar.SUCCESS)
        else:
    ...
    def show_info(self, title, content, icon):
        InfoBar.success(
            ...
            duration=2000,
            parent=self
        ) if icon == FIF.UP else InfoBar.warning(
            title=title,
            content=content,
            orient=Qt.Horizontal,
            isClosable=True,
