...
def remove_silence_from_end(input_file_path, silence_threshold=-50.0, chunk_size=10, is_start=True):
    ...
    if isinstance(input_file_path, str):
        ...
        if _format in ['wav', 'mp3', 'm4a']:
            audio = AudioSegment.from_file(input_file_path, format=_format if _format in ['wav', 'mp3'] else 'mp4')
        else:
            # 转为mp3
            try:
            ...
            if urls := [x for x in t if x.strip() and x.startswith('http')]:
                ...
                while n < 5:
                    ...
                    try:
                        ...
                        n += 1
                raise VideoProcessingError('从google.txt中随机获取5次url，均未找到可用的google翻译反代地址，请检查')
    return google_url
...
def format_result(source_list, target_list, target_lang="zh"):
    ...
    start = 0
    # 如果是中日韩泰语言，直接按字切割
    if (len(target_lang) < 6 and target_lang[:2].lower() in ['zh', 'ja', 'ko', 'th']) or (
            len(target_lang) > 5 and target_lang[:3].lower() in ['sim', 'tra', 'jap', 'kor', 'tha']):
