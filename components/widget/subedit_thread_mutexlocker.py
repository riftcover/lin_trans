...
from PySide6.QtCore import Qt, QAbstractTableModel, QModelIndex, QSize, QTimer, Signal, QObject, QMutex, QMutexLocker, QThread
...
class EditorCreatorWorker(QObject):
    """Worker class for creating editors in a separate thread"""
    editor_batch_ready = Signal(list)  # [(row, col), ...]
    batch_completed = Signal(int)  # current batch number
    finished = Signal()

    def __init__(self, model, batch_size=20):
        super().__init__()
        self.model = model
    ...
    def create_editors(self):
        """Create all editors in batches"""
        try:
            total_rows = self.model.rowCount()
            current_row = 0
            batch_number = 0

            while self.is_running and current_row < total_rows:
                with QMutexLocker(self.mutex):
                    ...
                    # 准备一批编辑器位置
                    batch = []
                    batch_end = min(current_row + self.batch_size, total_rows)
                    
                    for row in range(current_row, batch_end):
                        for col in range(self.model.columnCount()):
                            if col != 2:  # 跳过行号列
                                batch.append((row, col))
                    
                    if batch:
                        self.editor_batch_ready.emit(batch)
                        batch_number += 1
            ...
            logger.error(f"Error in create_editors: {e}")
            self.finished.emit()
...
class SubtitleTable(QTableView):
    ...
    def __init__(self, file_path: str):
        ...
        self.editor_worker = None
        self.editor_creation_started = False
    ...
    def create_visible_editors(self) -> None:
        """Start the editor creation process if not already started"""
        if self.editor_creation_started:
            return
            
        self.editor_creation_started = True
        self.start_editor_creation()

    def start_editor_creation(self):
        """Start the editor creation process in a separate thread"""
        self.cleanup_editor_thread()

        # 创建新的工作线程
        self.editor_thread = QThread()
        self.editor_worker = EditorCreatorWorker(self.model)
        self.editor_worker.moveToThread(self.editor_thread)

        # 连接信号
        self.editor_thread.started.connect(self.editor_worker.create_editors)
        self.editor_worker.editor_batch_ready.connect(self.create_editor_batch)
        self.editor_worker.batch_completed.connect(self.on_batch_completed)
        self.editor_worker.finished.connect(self.on_editor_creation_finished)
        self.editor_thread.finished.connect(self.editor_thread.deleteLater)

        # 启动线程
        self.editor_thread.start()
        logger.debug("Editor creation thread started")

    def create_editor_batch(self, batch):
        """Create a batch of editors in the main thread"""
    ...
    def on_batch_completed(self, batch_number):
        """Handle batch completion"""
        logger.debug(f"Completed batch {batch_number}")

    def on_editor_creation_finished(self):
        """Handle completion of editor creation"""
        logger.debug("Editor creation completed")
        if self.editor_worker:
            self.editor_worker.deleteLater()
            self.editor_worker = None
        if self.editor_thread:
            self.editor_thread.quit()
            self.editor_thread.wait()

    def cleanup_editor_thread(self):
        """Clean up existing editor thread"""
        if self.editor_worker:
            self.editor_worker.stop()
        if self.editor_thread and self.editor_thread.isRunning():
            self.editor_thread.quit()
            self.editor_thread.wait()
