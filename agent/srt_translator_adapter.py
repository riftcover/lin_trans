...
class SRTTranslatorAdapter:
    ...
    def split_entries_into_chunks(self, entries: List[SRTEntry], chunk_size: int = 600, max_entries: int = 10) -> List[List[SRTEntry]]:
        """将SRT条目分割为适合翻译的块"""
        chunks = []
        current_chunk = []
        current_length = 0
    ...
    def entries_to_text_chunks(self, entry_chunks: List[List[SRTEntry]]) -> List[str]:
        """将SRT条目块转换为纯文本块"""
        text_chunks = []
    ...
    def get_context_for_chunk(self, entry_chunks: List[List[SRTEntry]], chunk_index: int) -> Tuple[Optional[List[str]], Optional[List[str]]]:
        """获取指定块的上下文"""
        # 获取前一块的最后3个条目
    ...
    def rebuild_srt_from_translations(self, original_entries: List[SRTEntry], translations: List[str]) -> str:
        """将翻译结果重组为SRT格式"""
        ...
        for i, (entry, translation) in enumerate(zip(original_entries, translations)):
            # 保持原始的序号和时间戳
            srt_lines.append(entry.original_header)
            # 添加原文
            srt_lines.append(entry.text)
            # 添加译文
            srt_lines.append(translation.strip())
            # 添加空行分隔符（除了最后一个条目）
