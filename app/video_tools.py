...
class FFmpegJobs:
    ...
    @staticmethod
    def convert_ts_to_mp4(input_file:Path, output_file:Path):
        logger.info(f'convert ts to mp4: {input_file} -> {output_file}')
        # (
        #     ffmpeg
        #     .input(input_file)
        #     .output(output_file)
    ...
    @staticmethod
    def convert_mp4_to_war(input_file:Path, output_file:Path):
        logger.info(f'convert mp4 to war: {input_file} -> {output_file}')
        command = [
            'ffmpeg',
            '-i', input_file,
            '-loglevel', 'warning',
